...
（格式化、日期運算）。`@Binding` 與 closure prop 是**資料通道**，可以留。

**「計算」指的是從資料算出設計值，不是佈局。** 依可用空間做比例佈局
（進度條把 `ratio` 映射成寬度）**就是呈現本身**，不算違規 —— 但 `GeometryReader`
要登記進 `logic-exceptions.json` 並寫理由，沒登記的是硬錯。
靜默放行的例外會變成永久的例外。

由**檢查 18** 機器擋。這條規則在 2026-09-04 之前只是慣例 ——
而這一輪已經證明過兩次「沒有檢查在擋的規則會漂」。
...
**1 · 身分** —— 層級、職責、原型 id、獨立使用與容器責任、三件套路徑、**組成**（L2 才有）

**2 · 介面** ★ —— Props（名稱／型別／**值域**／必填／預設）、Slots、事件。
**這節決定規則一是否可能** —— props 不夠，應用層就會被迫硬編。
值域比型別重要：`style: TLTagStyle` 的資訊量是零，要寫出那幾種 case 叫什麼。

**3 · 變體** —— 互斥的樣式選擇，含變體之間哪些狀態不通用。

**4 · 狀態** ★ —— 照標準矩陣逐項回答，不適用寫 `N/A` ＋ 理由。
`<!-- states: … -->` 與 `<!-- themes: … -->` 兩行是機器讀的。

**5 · 度量與行為** —— 尺寸、內距、最小寬度、壓縮行為，全部寫 token 名。
含「文字與溢出」與「動態」兩個小節。

**6 · 配色** ★ —— 只列語意 token，不出現任何 hex。

**7 · 用法與禁用法** —— 何時用這個而不是那個（**易混淆**要兩邊都寫），明確禁止項。
每條禁令都要有「為什麼」，3–4 條是上限 —— 超過代表職責混了，該拆元件。

**8 · 變更紀錄** —— 偏離設計稿時記這裡，含原因。

> **這個專案不做無障礙。** `ARCHITECTURE.md` 明寫 `accessibilityIdentifier`
> 是為了測試與工具化、不是無障礙工程。規格裡沒有無障礙那一節，
> identifier 慣例需要記的話寫在第 2 節的事件旁。
...
3. **preview 內零字面樣式值** —— 顏色／字級／間距／圓角一律 `var(--…)`。
   ...
   **豁免是路徑白名單，不是判斷題**：只適用 `tokens/tokens.preview.html`、`index.html`
   與未來的 `foundations/*.html`（設計系統自身的展示頁 —— 色票要畫多大跟設計系統無關）。
   **`atoms/` `molecules/` `organisms/` 底下的元件 preview 一律不豁免。**
   **字面色值在所有檔案都不豁免。**

   為什麼是白名單：寫成「不代表任何元件的展示骨架」需要人判斷，而人在趕的時候
   會判成「這個 46px 也不代表元件啊」。按路徑判定，lint 擋得住，也不必爭論語意
...
| 3 | spec 宣告的 props 與 `init` 參數一致 —— 兩個方向都擋：宣告了 init 沒有的 prop、以及**宣告無 props 但 init 其實收參數** | §6.2 |
...
（「第 4 條」「第 7 條是 ratchet」），重排會讓那些指稱失效。表格輸出時依 id 排序。
